...
// const topbrandcloth = top_brands_cloth
//   .map((item, index) => {
//     return `
...
document.addEventListener("DOMContentLoaded", async () => {
  await mensCategoryTopBrands();
});

async function mensCategoryTopBrands() {
  try {
    ...
    const mensCategoryTopBrands = data.mensCategoryTopBrands;

    const mensCategoryTopBrandsHtml = mensCategoryTopBrands
      .map(
        (item) => `
        <a href="/darshit/${item.link}.html">
            <div class="dk_product-item">
    ...
    document.getElementById("mv_top_cloth_brand").innerHTML =
      mensCategoryTopBrandsHtml;
  } catch (error) {
...
// const MensExploremore = exploremore
//   .map((item, index) => {
//     return `
...
document.addEventListener("DOMContentLoaded", async () => {
  await mensCategoryExploreMore();
});

async function mensCategoryExploreMore() {
  try {
    const response = await fetch("./../data/db.json");
    ...
    const data = await response.json();
    const mensCategoryExploreMore = data.mensCategoryExploreMore;

    const mensCategoryExploreMoreHtml = mensCategoryExploreMore
      .map(
        (item) => `
          <div class="col-6 col-sm-4 col-md-4 col-lg-2 text-center category">
            <a href="/darshit/${item.link}.html">
    ...
    document.getElementById("mv_mens_explore_more").innerHTML =
      mensCategoryExploreMoreHtml;
  } catch (error) {
    console.error("Error loading categories:", error);
  }
}
...
document.addEventListener("DOMContentLoaded", async () => {
  await mensCategoryNewOnQuickcart();
});

async function mensCategoryNewOnQuickcart() {
  try {
    const response = await fetch("./../data/db.json");
    ...
    const data = await response.json();
    const mensCategoryNewOnQuickcart = data.mensCategoryNewOnQuickcart;

    const mensCategoryNewOnQuickcartHtml = mensCategoryNewOnQuickcart
      .map(
        (item) => `
           <div class="swiper-slide">
            <a href="/darshit/${item.link}.html">
    ...
    document.getElementById("dk_new_on_quick").innerHTML =
      mensCategoryNewOnQuickcartHtml;
  } catch (error) {
    console.error("Error loading categories:", error);
  }
}
...
const Men_trending_deals = mens_trending_dealing
  .map((item) => {
    return `
        <div class="swiper-slide">
...
document.getElementById("mv_mens_trending_deals").innerHTML =
  Men_trending_deals;
...
// const menscategory_acc = menscategory_accessories
//   .map((item, index) => {
//     return `
//         <div class="swiper-slide">
//             <a href="/darshit/${item.link}.html">
...
document.addEventListener("DOMContentLoaded", async () => {
  await mensCategoryAcc();
});

async function mensCategoryAcc() {
  try {
    const response = await fetch("./../data/db.json");
    ...
    const data = await response.json();
    const mensCategoryAcc = data.mensCategoryAcc;

    const mensCategoryAccHtml = mensCategoryAcc
      .map(
        (item) => `
             <div class="swiper-slide">
            <a href="/darshit/${item.link}.html">
    ...
    document.getElementById("mv_mens_acc").innerHTML = mensCategoryAccHtml;
  } catch (error) {
    console.error("Error loading categories:", error);
  }
}
...
// const menscategory_ex_box = menscategory_exporing_box
//   .map((item, index) => {
//     return `
...
document.addEventListener("DOMContentLoaded", async () => {
  await mensCategoryExporingBox();
});

async function mensCategoryExporingBox() {
  try {
    const response = await fetch("./../data/db.json");
    ...
    const data = await response.json();
    const mensCategoryExporingBox = data.mensCategoryExporingBox;

    const mensCategoryExporingBoxHtml = mensCategoryExporingBox
      .map(
        (item) => `
              <div class="col-md-6 mb-4">
            <div class="card text-white">
    ...
    document.getElementById("mv_explore_ethnic").innerHTML =
      mensCategoryExporingBoxHtml;
  } catch (error) {
    console.error("Error loading categories:", error);
  }
}
